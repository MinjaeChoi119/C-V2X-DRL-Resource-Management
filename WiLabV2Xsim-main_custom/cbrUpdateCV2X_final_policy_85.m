function [timeManagement,stationManagement,sinrManagement,CBRvalues,coex_cbrLTEonlyValues,CRvalue] = cbrUpdateCV2X_final_policy_85(timeManagement,vehiclesToConsider,stationManagement,positionManagement,sinrManagement,appParams,simParams,phyParams,outParams,outputValues)
...
if simParams.dcc_active
    ...
    %CRlimit = randsample(randomCRvalues,length(vehiclesToConsider),true); %for training
    ...
    CRlimit = (1*(simParams.rho<203*phyParams.cv2xCbrFactor) + 0.006*(simParams.rho>=203*phyParams.cv2xCbrFactor & simParams.rho<236*phyParams.cv2xCbrFactor) + 0.003*( simParams.rho>=236*phyParams.cv2xCbrFactor))*phyParams.cv2xCbrFactor;
    %CRlimit = 1*phyParams.cv2xCbrFactor;
    %CRlimit = 0.006*phyParams.cv2xCbrFactor;
...
% for data extraction
CRvalue=CRlimit * ones(1, length(vehiclesToConsider));
