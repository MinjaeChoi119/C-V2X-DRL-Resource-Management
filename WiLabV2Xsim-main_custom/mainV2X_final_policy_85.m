function [simValues,outputValues,appParams,simParams,phyParams,sinrManagement,outParams,stationManagement, culmulativeCBRvalues, culmulativeCRvalues, culmulativePRR] = mainV2X_final_policy_85(appParams,simParams,phyParams,outParams,simValues,outputValues,positionManagement)
...
culmulativeCBRvalues = zeros(2*simParams.rho, 1);  % 안재욱 2*rho개의 고정된 행, 처음에는 열이 없음(28~41)
tempCBRvalues = zeros(2*simParams.rho, 1);  % 임시 배열, 한 바퀴 돌 때마다 CBR 값을 저장
visitedEvents = false(2*simParams.rho, 1);  % 각 idEvent가 한 번씩 발생했는지 추적하는 배열, 2*rho를 다 돌았으면 초기화

culmulativeCRvalues = zeros(2*simParams.rho, 1); % CR 확인
tempCRvalues = zeros(2*simParams.rho, 1);

culmulativePRR = zeros(2*simParams.rho, 1);    % 차량 누적
tempPRR = zeros(2*simParams.rho, 1);    % 차량 확인   
visitedEvent = false(2*simParams.rho, 1);    % 다음 열

outputValues.calculationPRR = 0; % 계산을 위한 초기 정의
outputValues.receptionTot = 0; % 계산을 위한 초기 정의
outputValues.totalPacketTot = 0; %(28~41)
...
%             %% =========
        ...
        if ~isempty(stationManagement.activeIDsCV2X)
            vehiclesToConsider = stationManagement.activeIDsCV2X(stationManagement.cbr_subinterval(stationManagement.activeIDsCV2X)==thisSubInterval);
            [timeManagement,stationManagement,sinrManagement,stationManagement.cbrCV2Xvalues(vehiclesToConsider,ceil(timeEvent/simParams.cbrSensingInterval)),stationManagement.coex_cbrLteOnlyValues(vehiclesToConsider,ceil(timeEvent/simParams.cbrSensingInterval)),CRvalue] = ...
                cbrUpdateCV2X_final_policy_85(timeManagement,vehiclesToConsider,stationManagement,positionManagement,sinrManagement,appParams,simParams,phyParams,outParams,outputValues);
       

            % 안재욱 모든 vehiclesToConsider에 대한 CBR 값을 tempCBRvalues에 저장(192~209) 
            for idx = 1:length(vehiclesToConsider) % 현재 고려 중인 차량 개수 만큼
                vehicleID = vehiclesToConsider(idx); % 해당 차량에 CBR, CR 할당
                CBRvalues = sinrManagement.cbrCV2X(vehicleID);  % 해당 차량의 CBR 값 가져오기
                tempCBRvalues(vehicleID) = CBRvalues;  % 임시 배열에 기록
                CRvalues = CRvalue(idx); % 해당 차량의 CR값 가져오기
                tempCRvalues(vehicleID) = CRvalues; % 임시 배열에 기록
                visitedEvents(vehicleID) = true;  % 해당 vehicleID가 처리되었음을 기록
            end

            % 모든 idEvent(1-2*rho)가 한 번씩 처리되었는지 확인
            if all(visitedEvents)
                culmulativeCBRvalues = [culmulativeCBRvalues, tempCBRvalues];  % 임시 배열을 열로 추가
                tempCBRvalues = zeros(2*simParams.rho, 1);  % 임시 배열 초기화 (다음 바퀴를 위해)
                culmulativeCRvalues = [culmulativeCRvalues, tempCRvalues];
                tempCRvalues = zeros(2*simParams.rho, 1);
                visitedEvents = false(2*simParams.rho, 1);  % 방문 배열 초기화 (다음 바퀴를 위해)
            end
        end
    ...
    elseif abs(timeEvent-timeManagement.timeNextCV2X)<1e-8    % timeEvent == timeManagement.timeNextCV2X
        ...
        if timeManagement.ttiCV2Xstarts
            ...
            [phyParams,simValues,outputValues,sinrManagement,stationManagement,timeManagement] = ...
                mainCV2XtransmissionEnds_final_policy(appParams,simParams,phyParams,outParams,simValues,outputValues,timeManagement,positionManagement,sinrManagement,stationManagement);

            firstIteration = true; % 안재욱(265~287)
            for idx = 1:length(stationManagement.transmittingIDsCV2X) % 현재 전송 중인 차량들 개수 만큼
                % 처음 반복할 때만 receptionTot 및 totalPacketTot 업데이트 (만일 2개 차량에 대해서 성공한 비콧과 오류가 뜬 비콘을 계산 했을 때, 269, 270에 2번 계산되는 걸 방지)
                if firstIteration
                    outputValues.receptionTot = outputValues.receptionTot + outputValues.reception;
                    outputValues.totalPacketTot = outputValues.totalPacketTot + outputValues.totalPacket;
                    firstIteration = false;  % 업데이트 후 플래그를 false로 설정하여 한 번만 실행되도록 함
                end
    
                % PRR 계산 및 tempPRR 업데이트
                outputValues.calculationPRR = outputValues.reception/outputValues.totalPacket; % 해당 시점의 단일 PRR 계산 
                activeIDsTX = stationManagement.transmittingIDsCV2X(idx); % 해당 차량의 번호
                PRRvalues = outputValues.calculationPRR; % PRR 정의 
                tempPRR(activeIDsTX) = PRRvalues; % 임시 배열에 PRR 기록
                visitedEvent(activeIDsTX) = true; % 해당 차량에 대해 PRR 관측을 수행했다 표시
            end

            % 모든 activeIDsTX(1-2*rho)가 한 번씩 처리되었는지 확인
            if all(visitedEvent)
                culmulativePRR = [culmulativePRR, tempPRR];  % 임시 배열을 열로 추가
                tempPRR = zeros(2*simParams.rho, 1);  % 임시 배열 초기화 (다음 바퀴를 위해)
                visitedEvent = false(2*simParams.rho, 1);  % 방문 배열 초기화 (다음 바퀴를 위해)
            end
    ...
    elseif abs(timeEvent-timeManagement.timeNextPacket(idEvent))<1e-8   % timeEvent == timeManagement.timeNextPacket(idEvent)
        ...
        %[timeManagement, stationManagement] = dcc_dqn(idEvent, timeManagement, stationManagement, simParams, appParams, timeEvent);
        if generationInterval >= timeManagement.dcc_minInterval(idEvent)
            ...
            %[timeManagement, stationManagement] = dcc_dqn(idEvent, timeManagement, stationManagement, simParams, appParams);
...
% DQN PARAMETER  & DATA
outputValues.averageCBR = mean(culmulativeCBRvalues(:, 6:end), 'all');
outputValues.averageCR = mean(culmulativeCRvalues(:, 6:end), 'all');
outputValues.averagePRR = mean(culmulativePRR(:, 6:end), 'all');
outputValues.rePRR = outputValues.receptionTot / outputValues.totalPacketTot;
