function WiLabV2Xsim_final_dqn(varargin)
...
% Agent 파라미터 분리
agentIdx = find(strcmp(varargin, 'Agent'));
    if ~isempty(agentIdx)
        Agent = varargin{agentIdx + 1}; % Agent 값을 저장
        varargin([agentIdx, agentIdx + 1]) = []; % varargin에서 Agent 관련 항목 제거
    else
        Agent = []; % Agent가 제공되지 않은 경우
    end

% Simulator parameters and initial settings
[simParams,appParams,phyParams,outParams] = initiateParameters(varargin);
dcc_active_DDQN=1;
simParams.dcc_active_DDQN = dcc_active_DDQN;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Start Simulation
[simValues,outputValues,appParams,simParams,phyParams,sinrManagement,outParams,stationManagement, culmulativeCBRvalues, culmulativeCRvalues, culmulativePRR] = mainV2X_final_dqn(appParams,simParams,phyParams,outParams,simValues,outputValues,positionManagement, Agent);    
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
writematrix(culmulativeCBRvalues, fullfile(outParams.outputFolder, 'CBRvalues.xlsx')); % 안재욱-파일들 뽑아내는 곳 곳(381~387)
writematrix(culmulativeCRvalues, fullfile(outParams.outputFolder, 'CRvalues.xlsx'));
writematrix(culmulativePRR, fullfile(outParams.outputFolder, 'PRRvalues.xlsx'));

dataToWrite = [outputValues.averageCBR, outputValues.averageCR, outputValues.packetReceptionRatioCV2X];
writematrix(dataToWrite, fullfile(outParams.outputFolder, 'DATAvalues.xlsx'));
writematrix(outputValues.rePRR, fullfile(outParams.outputFolder, 'rePRRvalues.xlsx')); % 여기까지(381~387)
