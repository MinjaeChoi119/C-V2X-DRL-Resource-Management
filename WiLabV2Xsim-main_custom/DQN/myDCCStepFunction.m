...
if isMember
    if Action == 1
        average_prr = data{index, 4}; % Action이 1일 때 4번째 열 사용
        rho = data{index, 1};
        CBR = data{index, 2};
        LoggedSignals.State = [rho; CBR]; 
        NextObs = LoggedSignals.State ;     
    elseif Action == 0.03
        average_prr = data{index, 9}; % Action이 0.03일 때 9번째 열 사용
        rho = data{index, 6};
        CBR = data{index, 7};
        LoggedSignals.State = [rho; CBR]; 
        NextObs = LoggedSignals.State ; 
    elseif Action == 0.006
        average_prr = data{index, 14}; % Action이 0.006일 때 14번째 열 사용
        rho = data{index, 11};
        CBR = data{index, 12};
        LoggedSignals.State = [rho; CBR]; 
        NextObs = LoggedSignals.State ;
    elseif Action == 0.003
        average_prr = data{index, 19}; % Action이 0.003일 때도 14번째 열 사용
        rho = data{index, 16};
        CBR = data{index, 17};
        LoggedSignals.State = [rho; CBR]; 
        NextObs = LoggedSignals.State ;
