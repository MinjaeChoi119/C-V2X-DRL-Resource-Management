function [simValues,outputValues,appParams,simParams,phyParams,sinrManagement,outParams,stationManagement, culmulativeCBRvalues, culmulativeCRvalues, culmulativePRR] = mainV2X_final_dqn(appParams,simParams,phyParams,outParams,simValues,outputValues,positionManagement, Agent)
...
%             %% =========
        ...
        if ~isempty(stationManagement.activeIDsCV2X)
            ...
            [timeManagement,stationManagement,sinrManagement,stationManagement.cbrCV2Xvalues(vehiclesToConsider,ceil(timeEvent/simParams.cbrSensingInterval)),stationManagement.coex_cbrLteOnlyValues(vehiclesToConsider,ceil(timeEvent/simParams.cbrSensingInterval))] = ...
                cbrUpdateCV2X_final_dqn(timeManagement,vehiclesToConsider,stationManagement,positionManagement,sinrManagement,appParams,simParams,phyParams,outParams,outputValues, Agent);
        end


        if simParams.dcc_active_DDQN
            for idx = 1:length(vehiclesToConsider)
                vehicleID = vehiclesToConsider(idx); % 해당 차량에 CBR, CR 할당
                CBRvalues = sinrManagement.cbrCV2X(vehicleID); 
                tempCBRvalues(vehicleID) = CBRvalues; 
                observation = [simParams.rho; CBRvalues];
                [MaxQ,actionIdx,~] = getMaxQValue(Agent.critic,{observation});
                rate = getElementValue(Agent.actInfo,actionIdx); %rate가 CRlimit = {1 0.006 0.003}

                CRlimit = rate*phyParams.cv2xCbrFactor;
                ...
                tempCRvalues(vehicleID) = rate; % 임시 배열에 기록
